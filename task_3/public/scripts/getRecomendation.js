export function getRecommendation(date, members, db) {
  ...
  db.events = db.events.filter(
    event =>
      needDateTrigerStart < Date.parse(event.date.start) < needDateTrigerEnd
  );
  rooms.forEach(room => {
  ...
  floors.forEach(floor => {
    let recommendedRooms = rooms.filter(
      room => room.floor === floor.floorNumber
    );
    ...
    recommendedRooms.forEach(room => {
      let eventsInRoom = copyEvents.filter(
        event => event.room === parseInt(room.id, 10)
      );
      room.dateValid = true;
      ...
      eventsInRoom.forEach(eventInRoom => {
        let dateStartValid =
          Date.parse(date.start) >= Date.parse(eventInRoom.date.end);
        let dateEndValid =
          Date.parse(date.end) <= Date.parse(eventInRoom.date.start);
  ...
  if (allRecommendation.length !== 0) {
    ...
    copyEvents = copyEvents.filter(event => {
      let firstCondition =
        Date.parse(event.date.end) > Date.parse(date.start) &&
        Date.parse(date.end) > Date.parse(event.date.start);

      let secondCondition =
        Date.parse(date.start) <= Date.parse(event.date.start) &&
        Date.parse(date.end) >= Date.parse(event.date.end);
      ...
      rooms.forEach(room => {
        if (parseInt(room.id, 10) !== event.room) {
          ...
          room.events.forEach(eventInRoom => {
            ...
            let firstCondition =
              Date.parse(eventInRoom.date.end) > Date.parse(event.date.start) &&
              Date.parse(event.date.end) > Date.parse(eventInRoom.date.start);

            let secondCondition =
              Date.parse(event.date.start) <=
                Date.parse(eventInRoom.date.start) &&
              Date.parse(event.date.end) >= Date.parse(eventInRoom.date.end);
          ...
          if (enableTransfer) {
            if (Array.isArray(room.swap)) {
              let duplicate = room.swap.filter(
                item => item.eventMain === event.id
              );
    ...
    allSwaps.forEach(swap => {
      let room = db.rooms.filter(
        room => parseInt(swap.roomEvent, 10) === parseInt(room.id, 10)
      );
      // console.log(room);
      if (room[0] != null) {
    ...
    if (result.length !== 0) {
      ...
      roomsWithEvents.forEach(room => {
        ...
        let eventsAfterStart = events.filter(
          event => Date.parse(event.date.end) > Date.parse(date.start)
        );
        eventsAfterStart.forEach(eventAfterStart => {
          ...
          events.forEach(eventValidation => {
            ...
            let dateStartValid =
              newDate.start >= Date.parse(eventValidation.date.end);
            let dateEndValid =
              newDate.end <= Date.parse(eventValidation.date.start);
            if (
              (dateEndValid || dateStartValid) &&
              newDate.end < endTime.valueOf()
            ) {
